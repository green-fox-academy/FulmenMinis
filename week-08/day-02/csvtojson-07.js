...
var myMap = myArray.map(function(item) {
  var finalList = item.split(";");
  if (!nameList.includes(finalList[0])) {
    nameList.push(finalList[0]);
    return {name: finalList[0], id: finalList[1], color_code: finalList[2]}  
  }
});
var myMapCompact = myMap.filter(function(item){
  return item != undefined;
});

console.log(myMapCompact);