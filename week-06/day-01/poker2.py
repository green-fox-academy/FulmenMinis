...
class poker_hand():
    ...
    def __repr__(self):
        ...
        # Full house or 4-of-a-kind
            ...
            if len(suit_dict) == 1:
                flush = True
            min_numeral = min([num_list.index(x) for x in numeral_dict.keys()])
            max_numeral = max([num_list.index(x) for x in numeral_dict.keys()])
...
class deck(set):
    def __init__(self):
        for numeral, suit in itertools.product(num_list, suit_list):
